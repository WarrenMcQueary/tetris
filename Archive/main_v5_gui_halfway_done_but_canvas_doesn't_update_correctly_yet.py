...
# TODO: Show a preview of the next block!
...
def take_input():
    ...
    #response = input("Choose a (left), d (right), s (down), q (rotate ccw), or e (rotate cw): ")   # TODO: Re-enable this line, and disable the lines below.
    time.sleep(0.5)
    response = "s"
...
def draw_game_canvas(cellsize):
    """Updates the game canvas with the current boarrd state.
    cellsize is a single int representing the width and height of each cell, in pixels.
    """
    global grid
    ...
    for row in range(np.shape(grid_sans_buffer)[0]+1):    # For each row, minus buffer rows, plus 1...
        game_canvas.create_line(0, row*cellsize, game_canvas["width"], row*cellsize)
    # Draw grid columns
    for column in range(np.shape(grid_sans_buffer)[1]+1): # For each column, plus 1...
        game_canvas.create_line(column*cellsize, 0, column*cellsize, game_canvas["height"])
    # Fill with pieces.
    for row in range(np.shape(grid_sans_buffer)[0]):    # For each row, minus buffer rows...
        for column in range(np.shape(grid_sans_buffer)[1]):     # For each column...
            if grid_sans_buffer[row, column] == 2:  # If there's a live piece in this cell...
                game_canvas.create_rectangle(column*cellsize, row*cellsize, (column+1)*cellsize, (row+1)*cellsize, fill="blue")
            elif grid_sans_buffer[row, column] == 1:    # Elif there's a dead piece in this cell...
                game_canvas.create_rectangle(column*cellsize, row*cellsize, (column+1)*cellsize, (row+1)*cellsize, fill="cyan")
            else:   # Else (this cell must be empty)...
                pass
    pass    # TODO: Delete this debugging line.
...
def game_loop():
    ...
    while True:
        if not create_piece(possible_pieces):  # If creating the new piece does not cause a loss:
            while True:
                print(grid[2:, :])  # TODO: Disable this after debugging
                draw_game_canvas(cell_size)
...
window.mainloop()
# TODO: Call draw_game_canvas when appropriate.
# TODO: Switch to keyboard event input.
